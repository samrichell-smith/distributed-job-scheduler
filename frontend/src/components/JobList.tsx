        ...
        <tbody>
          {jobs.map((job) => (
            <tr key={job.id} className="border-b hover:bg-blue-50 transition">
              <td className="py-2 px-4 font-mono text-xs text-blue-700 truncate max-w-xs">{job.id}</td>
              ...
              <td className="py-2 px-4">{new Date(job.created_at).toLocaleString()}</td>
              <td className="py-2 px-4">{job.started_at ? new Date(job.started_at).toLocaleString() : "-"}</td>
              <td className="py-2 px-4">{job.completed_at ? new Date(job.completed_at).toLocaleString() : "-"}</td>
            </tr>
          ))}
        </tbody>
      </table>
